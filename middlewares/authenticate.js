...
module.exports = async (req, res, next) => {
  try {
    console.log('headers', req.body, req.headers)
    const reqPath = req.originalUrl
    const headers = req.body.headers.Authorization
    ...
    if(id_user){
      // if (reqPath.startsWith('/users/create') || reqPath.startsWith('/users/delete') || reqPath.startsWith('/users/update')) {
        if (reqPath.startsWith('/users/delete') || reqPath.startsWith('/users/update')) {
      ...
      } else if (reqPath === '/users' || reqPath.startsWith('/users/create')) {
